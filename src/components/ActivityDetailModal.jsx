...
export default function ActivityDetailModal({ user, open, onClose }) {
    ...
    (async () => {
      try {
        const [a, s] = await Promise.all([
          fetchDailyActivity(user.id, todayISO()),
          fetchScore(user.id, todayISO()),
        ]);
        setActivity(a);
        setScore(s);
      } finally {
  ...
  return (
    <Modal open={open} onClose={onClose} title={user ? `Aktivitas — ${user.name}` : ''} maxWidth="max-w-xl">
      {loading ? (
        <FullSpinner />
      ) : !activity?.activities?.length ? (
        <p className="text-sm text-text-secondary text-center py-6">Belum ada aktivitas hari ini.</p>
      ) : (
