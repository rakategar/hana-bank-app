...
export default function ScoreBadge({ score, level, size = 'sm', showScore = false, className = '' }) {
  ...
  if (!info) {
    return (
      <span className={clsx('px-3 py-1 rounded-full text-[11px] font-semibold bg-text-muted/15 text-text-secondary', className)}>
        BELUM DINILAI
      </span>
    );
  }
  ...
  return (
    <span
      className={clsx('inline-flex items-center gap-1 rounded-full font-semibold uppercase tracking-wide', sizeCls, className)}
