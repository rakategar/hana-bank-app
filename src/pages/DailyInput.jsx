...
import {
  ...
  serializeStructuredData, isStructuredFilled, getDailyInputGraceDates, isDailyInputGraceEnabled,
} from '../lib/utils';
...
export default function DailyInput() {
  ...
  async function handleCreateExtraPlan(payload) {
    ...
    logActivity({ userId: user.id, role: user.role, action: 'extra_plan_created', entityDate: payload.date });
    const noWeeklyPlan = user.role === 'FWSS' || user.role === 'BM';
    const weekId = currentWeekId(new Date(selectedDate + 'T12:00:00'));
  ...
  const persist = useCallback(
    async (overrideSlots, opts = {}) => {
      ...
      // Log draft hanya sekali per sesi tanggal (pertama kali auto-save berhasil)
      if ((!opts.status || opts.status === 'draft') && !draftLoggedRef.current) {
        draftLoggedRef.current = true;
        logActivity({ userId: user.id, role: user.role, action: 'daily_input_draft', entityDate: selectedDate });
      }
      setSavedAt(new Date());
      dirty.current = false;
      setIsDirty(false);
  ...
  async function handleSaveSlot(idx) {
    ...
    try {
      await persist();
      logActivity({ userId: user.id, role: user.role, action: 'daily_input_slot_saved', entityDate: selectedDate, metadata: { time: slot.time } });
  ...
  async function handleSubmitScore() {
    ...
    // Batalkan auto-save yang tertunda agar tidak menimpa status 'submitted'/'scored'
    // dengan 'draft' di tengah proses scoring (race condition).
    clearTimeout(autoSaveTimer.current);
    dirty.current = false;
    setIsDirty(false);
    setBusy('scoring');
    ...
    try {
      ...
      logActivity({ userId: user.id, role: user.role, action: 'daily_input_submitted', entityDate: selectedDate });
      ...
      logActivity({ userId: user.id, role: user.role, action: 'score_completed', entityDate: selectedDate, metadata: { daily_level: result?.daily_level } });
  ...
  const alwaysOpenDaily = isDailyInputGraceEnabled();
          ...
          {viewSlots.length > 0 ? (
            <div className="grid lg:grid-cols-2 gap-4">
              {viewSlots.map((slot) => {
                ...
                // Flag VITE_DAILY_INPUT_GRACE=yes → slot hari ini juga selalu open (tanpa time-gating).
                // Flag=no/kosong (live ketat): hari ini dihitung dari waktu nyata.
                const windowState = (!isToday || alwaysOpenDaily)
                  ? 'open'
