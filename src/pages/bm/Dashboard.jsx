...
export default function BMDashboard() {
  ...
  const [tree, setTree] = useState([]); // [{ fwss snapshot, fas: [snapshot] }]
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="text-sm text-text-muted">Branch Manager · {user.branch}</p>
          </div>
          ...
          <div>
            <SectionTitle>Monitoring FWSS &amp; FA</SectionTitle>
            {teamLoading ? (
              <div className="grid gap-3"><div className="skeleton h-24" /><div className="skeleton h-24" /></div>
            ) : (
              <div className="space-y-4">
                {tree.map(({ fwss, fas }) => (
          ...
          <button onClick={() => navigate('/summary/bm')} className="btn-teal w-full">
            <Bot size={18} /> Generate Summary Tim
          </button>
        </div>
      )}
